...
function git_status()
{
    return git_execute("sudo git status");
}

function git_checkout($tag)
{
    return git_execute("sudo git fetch && sudo git checkout $tag");
}

function git_execute($cmd)
{
    $path = __DIR__;    //your path
    $log_file = $path . '/runtime/deploy-error-output.log';

    error_log("Deploy execute `$cmd` at " . date('Y-m-d H:i:s'), 3, $log_file);
